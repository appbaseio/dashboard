...
class AppDashboard {
	constructor() {
		this.allowedView = ['dashboard', 'configurator', 'browser', 'mappings', 'builder', 'credentials', 'team'];
	}
		...
		const getMetrics = () => {
			appbaseService
				...
				.then(data => {
					...
					if (
						info.metrics.body &&
						info.metrics.body.month &&
						info.metrics.body.month.buckets
					) {
						const buckets = info.metrics.body.month.buckets;
						if (
							buckets[buckets.length - 1] &&
							buckets[buckets.length - 1].key_as_string
						) {
