...
export default class CloneApp extends Component {
		...
		if (nextProps.app && nextProps.app.permissions && nextProps.app.permissions.length) {
			const cloneAppLink = {
				...
				importFrom: {
					...
					hosturl: `https://${nextProps.app.permissions[0].username}:${
						nextProps.app.permissions[0].password
					}@scalr.api.appbase.io`,
				},
			};
			this.setState({
				cloneAppLink: `https://opensource.appbase.io/dejavu/importer?app=${JSON.stringify(
					cloneAppLink,
				)}`,
	...
	render() {
		return (
			<button
				aria-label="Clone this app"
				onClick={this.cloneApp}
				{...common.isDisabled(!this.state.cloneAppLink)}
				className="ad-theme-btn-transparent ad-list-app-header-clone card-icon pointer"
			>
