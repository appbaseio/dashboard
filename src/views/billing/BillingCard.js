...
export default class BillingCard extends Component {
	...
	renderElement(method) {
		...
		switch (method) {
			...
			case 'freeDescription':
				generatedEle = (
					<CardContent.FreeDescription {...this.props} cardInfo={this.cardInfo} />
				);
				break;
			case 'bootstrapHeader':
				generatedEle = (
					<CardContent.BootstrapHeader {...this.props} cardInfo={this.cardInfo} />
				);
				break;
			case 'bootstrapDescription':
				generatedEle = (
					<CardContent.BootstrapDescription {...this.props} cardInfo={this.cardInfo} />
				);
				break;
			case 'growthHeader':
				generatedEle = (
					<CardContent.GrowthHeader {...this.props} cardInfo={this.cardInfo} />
				);
				break;
			case 'growthDescription':
				generatedEle = (
					<CardContent.GrowthDescription {...this.props} cardInfo={this.cardInfo} />
				);
				break;
		}
		return generatedEle;
	...
	changeSubscribe() {
		...
		if (this.props.plan === 'free') {
			this.updateCustomer();
		} else if (this.customerCopy.hasOwnProperty('stripeKey') && this.customerCopy.stripeKey) {
	...
	updateCustomer() {
		...
		billingService
			...
			.then(resData => {
				...
				let finalAmount = this.billingText.payment.amount - this.billingText.refund.amount;
	...
	render() {
		const cx = classNames({
			subscribed:
				this.props.plan === this.props.activePlan &&
				this.props.mode === this.props.activeMode,
		});
		const loadingCondition = this.props.loading === this.props.plan || this.state.localLoading;
		return (
			<div
				className="col-xs-12 col-sm-12 single-card-container"
				id={`${this.props.plan}-card`}
			>
				<div className="price-card">
					...
					<div className="button text-center plan-button">
						<button
							{...common.isDisabled(loadingCondition)}
							className={`pos-relative new-btn get-started ${cx}`}
							onClick={this.changeSubscribe}
						>
							{loadingCondition ? <Loading /> : null}
							<span>
								{' '}
								{this.props.activePlan === this.props.plan &&
								this.props.mode === this.props.activeMode
									? 'Current plan'
									: 'Subscribe plan'}{' '}
							</span>
